...
_iEtsh_.logo = (() => {
    ...
    const installUpdateMonitorEnhancement = () => {
        ...
        const applyDetails = async () => {
            const list = document.querySelector('.update-monitor-list');
            if (!list) return;
            ...
            if (!items.length || items.length === lastCount) return;
            ...
            try {
                const clearAt = Number(
                    localStorage.getItem('ietsh-lmd-update-cleared-at')
                ) || 0;
                if (clearAt) {
                    visibleEntries = cachedEntries.filter(entry => {
                        const time = new Date(entry.time).getTime();
                        return Number.isNaN(time) || time > clearAt;
                    });
                }
            } catch (error) {
            ...
            items.forEach(item => {
                if (item.querySelector('.update-entry-change')) return;

                const typeText = normalize(
                    item.querySelector('.update-entry-type')?.textContent
                );
                const puzzleText = normalize(
                    item.querySelector('.update-entry-puzzle')?.textContent
                );
                const titleText = normalize(
                    item.querySelector('.update-entry-title')?.textContent
                );

                const matchIndex = visibleEntries.findIndex((entry, index) => {
                    if (used.has(index)) return false;

                    const entryType = normalize(entry.type);
                    const entryTitle = normalize(entry.title);
                    const entryPuzzle = normalize(`#${entry.puzzleNum}`);

                    return (
                        entryType === typeText &&
                        entryTitle === titleText &&
                        puzzleText.includes(entryPuzzle)
                    );
                });
                ...
                used.add(matchIndex);
                const entry = visibleEntries[matchIndex];

                const change = document.createElement('div');
                change.className = 'update-entry-change';

                const previous = document.createElement('div');
                previous.className = 'update-entry-value previous';
                previous.textContent = formatValue(entry.previous);

                const arrow = document.createElement('div');
                arrow.className = 'update-entry-arrow';
                arrow.textContent = '→';
                arrow.setAttribute('aria-hidden', 'true');

                const current = document.createElement('div');
                current.className = 'update-entry-value current';
                current.textContent = formatValue(entry.current);

                change.append(previous, arrow, current);
        ...
        const start = () => {
            const dashboard = document.querySelector('#puzzle-dashboard');
            if (!dashboard) {
